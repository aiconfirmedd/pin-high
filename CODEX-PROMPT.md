...
- **Supabase** for auth, round sync, admin data
- **CSS design tokens** in `src/index.css` (no Tailwind, no CSS modules)
- **Deployment**: Vercel (auto-deploy on push to `main`)
- **Owner email**: `aiconfirmedd@gmail.com` — triggers admin mode

## Design System (non-negotiable)

Read `.claude/skills/frontend-design/SKILL.md` for the full system. Summary:

- **Background**: `#18181A` (near-black steel)
- **Accent**: copper `#C4762A` → `#E0A550` gradient
- **Font**: `'Oswald'` for headings, system-ui for body
- **Tone**: luxury dark metal — like a premium golf watch
- **NEVER**: white backgrounds, blue buttons, Inter/Roboto for headings, purple gradients
...
- Selected segment highlights copper (`var(--grad-copper)` fill)
...
- Segments: dark steel fill (`var(--surface2)`), copper stroke on active
...
- Canvas (or SVG) rendering a top-down putting green:
  ...
  - Pin position marker at default center-right: copper flag icon (8px circle + line)
  - Fringe indicated by slightly lighter green ring
- Tap anywhere on green to place an approach landing marker:
  - First tap: places shot landing (white circle, 10px radius, copper stroke)
...
- Visual state: mic button pulses copper while listening
...
- Show parsed results in a confirmation strip at top: "Score: 4 · FIR ✓ · GIR ✗" — tap to confirm and populate fields
...
**Styling**:
- Mic button: 36px, `var(--surface2)` background, copper mic icon
- Listening state: `animation: pulse 1s infinite` — copper glow ring
- Confirmation strip: slides down from top, dark card, green checkmarks, copper values
...
### `src/App.tsx` — Round sync
`handleRoundChange(round)` already handles Supabase upsert. The `holes` column is JSONB — all new fields auto-persist. No schema changes needed.
...
- `.cta-btn` — copper gradient primary button
...
5. Match the copper/steel aesthetic exactly — no improvised colors
